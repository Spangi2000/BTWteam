...
from mnk_backend.exceptions import DateRangeError
from mnk_backend.models.db import Strike
from mnk_backend.schemas.models import StrikeGet, StrikePost
...
async def create_strike(
    strike_info: StrikePost, user=Depends(UnionAuth(scopes=["mnk.strike.create"], allow_none=False))
) -> StrikeGet:
...
async def get_strikes(
    ...
    user=Depends(UnionAuth(scopes=["mnk.strike.read"], allow_none=False)),
) -> list[StrikeGet]:
...
@strike.delete("/{id}")
async def delete_strike(id: int, user=Depends(UnionAuth(scopes=["mnk.strike.delete"], allow_none=False))) -> dict:
