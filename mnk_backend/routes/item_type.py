...
from mnk_backend.exceptions import ObjectNotFound
from mnk_backend.models.db import Item, ItemType
from mnk_backend.schemas.base import StatusResponseModel
from mnk_backend.schemas.models import ItemTypeGet, ItemTypePost
from mnk_backend.settings import Settings, get_settings
...
async def create_item_type(
    ...
    user=Depends(UnionAuth(scopes=["mnk.item_type.create"], allow_none=False)),
) -> ItemTypeGet:
    """
...
async def update_item_type(
    id: int, item_type_info: ItemTypePost, user=Depends(UnionAuth(scopes=["mnk.item_type.update"], allow_none=False))
) -> ItemTypeGet:
    """
...
async def delete_item_type(
    id: int, user=Depends(UnionAuth(scopes=["mnk.item_type.delete"], allow_none=False))
) -> StatusResponseModel:
    """
