...
from mnk_backend import settings
...
from mnk_backend.schemas.models import ItemGet, ItemPost
...
async def create_item(item: ItemPost, user=Depends(UnionAuth(scopes=["mnk.item.create"]))) -> ItemGet:
    """
...
async def update_item(
    ...
    user=Depends(UnionAuth(scopes=["mnk.item.patch"])),
) -> ItemGet:
    """
...
async def delete_item(
    id: int, user=Depends(UnionAuth(scopes=["mnk.item.delete"], allow_none=False))
) -> StatusResponseModel:
    """
